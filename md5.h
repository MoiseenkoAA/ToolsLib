...
public:
    ...
    CMaaString GetTextHash() // 16 bytes string
    {
        return GetHash().DisplayHex();
    }
#if TOOLSLIB_USE_CMAASTRING64 == 2
    CMaaString64 GetHash64() // 16 bytes string
    {
        CMaaString64 digest(nullptr, 16);
        if (digest.Length() == 16)
        {
            GetHash((char*)(const char*)digest);
        }
    ...
    CMaaString64 GetTextHash64() // 16 bytes string
    {
        return GetHash64().DisplayHex();
    }
#endif
};
